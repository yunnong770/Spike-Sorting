...
channel_select= 3; % select channel for testing. channel_select<= channel_number
...
chan1.filt_sig = wden(chan1.filt_br, 'rigrsure', 'h', 'one', 6, 'db4'); % Wavelet denoising
plot(chan1.filt_sig)
...
vNEO = [0; vNEO; 0];
[peaks_energy, locs_energy] = findpeaks(vNEO, 'MinPeakHeight', 0.3, 'MinPeakDistance', 18);
locs_spikes = [];
...
feature = feature(:,10:30);
plot(feature.')

%% Cluster spikes
group_mat = [];
for j = 1:10
    Y = tsne(feature, 'Exaggeration', 2, 'NumDimensions', 2, 'Perplexity', 4, 'LearnRate', 100);
    s = [];
    idx_mat = [];
    for i = 3:20
